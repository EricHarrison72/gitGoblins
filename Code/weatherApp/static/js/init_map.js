...
function initMap() {
    ...
    createMarker(map, -33.8751, 150.7634, 'BadgerysCreek');
    createMarker(map, -31.4978, 145.8363, 'Cobar');
    createMarker(map, -30.2962, 153.1139, 'CoffsHarbour');
    createMarker(map, -29.4653, 149.8416, 'Moree');
    createMarker(map, -32.9283, 151.7817, 'Newcastle');
    createMarker(map, -33.2822, 151.5669, 'NorahHead');
    createMarker(map, -29.0408, 167.9547, 'NorfolkIsland');
    ...
    createMarker(map, -33.9400, 151.1754, 'SydneyAirport');
    createMarker(map, -35.1026, 147.3655, 'WaggaWagga');
    ...
    createMarker(map, -35.5294, 148.7723, 'MountGinini');
    ...
    createMarker(map, -37.6708, 144.8430, 'MelbourneAirport');
    ...
    createMarker(map, -28.0167, 153.4000, 'GoldCoast');
    createMarker(map, -19.2590, 146.8169, 'Townsville');
    createMarker(map, -34.9285, 138.6007, 'Adelaide');
    createMarker(map, -37.8284, 140.7807, 'MountGambier');
    ...
    createMarker(map, -31.6676, 116.0292, 'PearceRAAF');
    createMarker(map, -31.9385, 115.9672, 'PerthAirport');
    createMarker(map, -31.9514, 115.8617, 'Perth');
    createMarker(map, -32.9804, 121.6456, 'SalmonGums');
    ...
    createMarker(map, -23.6980, 133.8807, 'AliceSprings');
